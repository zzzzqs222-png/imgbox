...
        // 特殊操作：获取全局索引信息（sum=true 且 count 不为 -1）
        // 用于获取索引的健康状态、总文件数等信息
        if (sum && count !== -1) {
...
        // 普通查询：只返回总数（count=-1 且 sum=true）
        if (count === -1 && sum) {
            result = await readIndex(context, {
                search,
                directory: dir,
                channel,
                listType,
...
        // 索引读取失败，直接从 KV 中获取所有文件记录（回退机制）
        if (!result.success) {
            console.warn(`Index read failed, falling back to KV: ${result.error}`);
